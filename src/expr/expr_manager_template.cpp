...
#define INC_STAT_VAR(type, bound_var)                                    \
  {                                                                      \
    ...
    TypeConstant type = typeNode->getKind() == kind::TYPE_CONSTANT       \
...
ExprManager::ExprManager() : d_nodeManager(new NodeManager(this)) {
#ifdef CVC4_STATISTICS_ON
  ...
  try {
    ...
    for (unsigned i = 0; i < kind::LAST_KIND; ++i) {
      if (d_exprStatistics[i] != NULL) {
        d_nodeManager->getStatisticsRegistry()->unregisterStat(
            d_exprStatistics[i]);
...
BooleanType ExprManager::booleanType() const {
  NodeManagerScope nms(d_nodeManager);
  return BooleanType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->booleanType())));
}

StringType ExprManager::stringType() const {
  NodeManagerScope nms(d_nodeManager);
  return StringType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->stringType())));
}
...
IntegerType ExprManager::integerType() const {
  NodeManagerScope nms(d_nodeManager);
  return IntegerType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->integerType())));
}

RoundingModeType ExprManager::roundingModeType() const {
  NodeManagerScope nms(d_nodeManager);
  return RoundingModeType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->roundingModeType())));
}
...
Expr ExprManager::mkExpr(Kind kind, Expr child1) {
  ...
  PrettyCheckArgument(n >= minArity(kind) && n <= maxArity(kind), kind,
                      ...
                      kind::kindToString(kind).c_str(), minArity(kind),
                      maxArity(kind), n);
  NodeManagerScope nms(d_nodeManager);
  try {
    INC_STAT(kind);
...
Expr ExprManager::mkExpr(Kind kind, Expr child1, Expr child2, Expr child3) {
  ...
  try {
    INC_STAT(kind);
    return Expr(
        this, d_nodeManager->mkNodePtr(kind, child1.getNode(), child2.getNode(),
                                       child3.getNode()));
  } catch (const TypeCheckingExceptionPrivate& e) {
    throw TypeCheckingException(this, &e);
  }
...
Expr ExprManager::mkExpr(Kind kind, const std::vector<Expr>& children) {
  const kind::MetaKind mk = kind::metaKindOf(kind);
  const unsigned n =
      children.size() - (mk == kind::metakind::PARAMETERIZED ? 1 : 0);
  PrettyCheckArgument(
      mk == kind::metakind::PARAMETERIZED || mk == kind::metakind::OPERATOR,
      kind,
      "Only operator-style expressions are made with mkExpr(); "
  ...
  std::vector<Expr>::const_iterator it_end = children.end();
  while (it != it_end) {
...
Expr ExprManager::mkExpr(Kind kind, Expr child1,
  ...
  std::vector<Expr>::const_iterator it = otherChildren.begin();
  std::vector<Expr>::const_iterator it_end = otherChildren.end();
  while (it != it_end) {
    nodes.push_back(it->getNode());
    ++it;
  }
...
Expr ExprManager::mkExpr(Expr opExpr) {
  ...
  PrettyCheckArgument(
      ...
      opExpr, "This Expr constructor is for parameterized kinds only");
  PrettyCheckArgument(n >= minArity(kind) && n <= maxArity(kind), kind,
                      "Exprs with kind %s must have at least %u children and "
                      "at most %u children (the one under construction has %u)",
                      kind::kindToString(kind).c_str(), minArity(kind),
...
Expr ExprManager::mkExpr(Expr opExpr, Expr child1) {
  ...
  try {
    INC_STAT(kind);
    return Expr(this,
                d_nodeManager->mkNodePtr(opExpr.getNode(), child1.getNode()));
  } catch (const TypeCheckingExceptionPrivate& e) {
    throw TypeCheckingException(this, &e);
  }
}
...
Expr ExprManager::mkExpr(Expr opExpr, Expr child1, Expr child2, Expr child3,
  ...
  try {
    INC_STAT(kind);
    return Expr(this, d_nodeManager->mkNodePtr(
                          opExpr.getNode(), child1.getNode(), child2.getNode(),
                          child3.getNode(), child4.getNode()));
  } catch (const TypeCheckingExceptionPrivate& e) {
    throw TypeCheckingException(this, &e);
  }
...
bool ExprManager::hasOperator(Kind k) { return NodeManager::hasOperator(k); }
...
FunctionType ExprManager::mkFunctionType(Type domain, Type range) {
  NodeManagerScope nms(d_nodeManager);
  return FunctionType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->mkFunctionType(
                              *domain.d_typeNode, *range.d_typeNode))));
}
...
FunctionType ExprManager::mkFunctionType(const std::vector<Type>& sorts) {
  ...
  return FunctionType(Type(
      d_nodeManager, new TypeNode(d_nodeManager->mkFunctionType(sortNodes))));
}
...
FunctionType ExprManager::mkPredicateType(const std::vector<Type>& sorts) {
  ...
  return FunctionType(Type(
      d_nodeManager, new TypeNode(d_nodeManager->mkPredicateType(sortNodes))));
}
...
DatatypeType ExprManager::mkTupleType(const std::vector<Type>& types) {
  ...
  return DatatypeType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->mkTupleType(typeNodes))));
}

DatatypeType ExprManager::mkRecordType(const Record& rec) {
  NodeManagerScope nms(d_nodeManager);
  return DatatypeType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->mkRecordType(rec))));
}
...
SExprType ExprManager::mkSExprType(const std::vector<Type>& types) {
  ...
  return SExprType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->mkSExprType(typeNodes))));
}

FloatingPointType ExprManager::mkFloatingPointType(unsigned exp,
                                                   unsigned sig) const {
  NodeManagerScope nms(d_nodeManager);
  return FloatingPointType(
      Type(d_nodeManager,
           new TypeNode(d_nodeManager->mkFloatingPointType(exp, sig))));
}

BitVectorType ExprManager::mkBitVectorType(unsigned size) const {
  NodeManagerScope nms(d_nodeManager);
  return BitVectorType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->mkBitVectorType(size))));
}

ArrayType ExprManager::mkArrayType(Type indexType, Type constituentType) const {
  NodeManagerScope nms(d_nodeManager);
  return ArrayType(Type(
      d_nodeManager, new TypeNode(d_nodeManager->mkArrayType(
                         *indexType.d_typeNode, *constituentType.d_typeNode))));
}

SetType ExprManager::mkSetType(Type elementType) const {
  NodeManagerScope nms(d_nodeManager);
  return SetType(Type(d_nodeManager, new TypeNode(d_nodeManager->mkSetType(
                                         *elementType.d_typeNode))));
}
...
class RefCountedBackedge : public Backedge {
public:
  RefCountedBackedge(NodeValue* nv)
    : d_nv(nv) {
    Assert(d_nv != NULL);
    d_nv->inc();
  }
  virtual ~RefCountedBackedge() {
...
class DatatypeBackedge : public RefCountedBackedge {
public:
  DatatypeBackedge(NodeValue* nv, Datatype* data)
    : RefCountedBackedge(nv), d_datatype(data) {}

  virtual ~DatatypeBackedge() {
    // This class does not own d_dataype. Do nothing.
  }

  virtual void removeBackedge() {
    d_datatype->unresolve();
  }
...
class DatatypeBackedgeFactory {
public:
  Backedge* operator()(NodeValue* nv, T* data) {
    return new DatatypeBackedge(nv, data);
  }
};
...
std::vector<DatatypeType> ExprManager::mkMutualDatatypeTypes(
  ...
  typedef std::vector<Datatype>::const_iterator DatatypesIterator;
  ...
  // pred selector.
  for (DatatypesIterator i = datatypes.begin(), i_end = datatypes.end();
       i != i_end; ++i) {
    TypeNode* typeNode;
    if ((*i).getNumParameters() == 0) {
      ...
      typeNode = new TypeNode(
          d_nodeManager->mkTypeNode(kind::PARAMETRIC_DATATYPE, params));
    }
  ...
  for (std::vector<DatatypeType>::iterator i = dtts.begin(), i_end = dtts.end();
       i != i_end; ++i) {
    DatatypeType& current_type = *i;
    const Datatype& dt = current_type.getDatatype();
    if (!dt.isResolved()) {
      // We break the encapsulation of the current_type.
      Datatype* mutable_dt = const_cast<Datatype*>(&dt);
      mutable_dt->resolve(this, nameResolutions, placeholders, replacements,
  ...
  for (std::vector<NodeManagerListener*>::iterator i =
           d_nodeManager->d_listeners.begin();
       i != d_nodeManager->d_listeners.end(); ++i) {
...
void ExprManager::checkResolvedDatatype(DatatypeType dtt) const {
  ...
  for (Datatype::const_iterator i = dt.begin(), i_end = dt.end(); i != i_end;
       ++i) {
...
ConstructorType ExprManager::mkConstructorType(
    const DatatypeConstructor& constructor, Type range) const {
  NodeManagerScope nms(d_nodeManager);
  return Type(d_nodeManager, new TypeNode(d_nodeManager->mkConstructorType(
                                 constructor, *range.d_typeNode)));
}

SelectorType ExprManager::mkSelectorType(Type domain, Type range) const {
  NodeManagerScope nms(d_nodeManager);
  return Type(d_nodeManager, new TypeNode(d_nodeManager->mkSelectorType(
                                 *domain.d_typeNode, *range.d_typeNode)));
}
...
TesterType ExprManager::mkTesterType(Type domain) const {
  ...
  return Type(d_nodeManager,
              new TypeNode(d_nodeManager->mkTesterType(*domain.d_typeNode)));
}

SortType ExprManager::mkSort(const std::string& name, uint32_t flags) const {
  NodeManagerScope nms(d_nodeManager);
  return SortType(
      Type(d_nodeManager, new TypeNode(d_nodeManager->mkSort(name, flags))));
}
...
Type ExprManager::mkSubrangeType(const SubrangeBounds& bounds) throw(
    TypeCheckingException) {
  NodeManagerScope nms(d_nodeManager);
  try {
    return SubrangeType(Type(
        d_nodeManager, new TypeNode(d_nodeManager->mkSubrangeType(bounds))));
  } catch (const TypeCheckingExceptionPrivate& e) {
    throw TypeCheckingException(this, &e);
  }
}
...
Expr ExprManager::mkAssociative(Kind kind, const std::vector<Expr>& children) {
  ...
  while (it != end && numChildren > max) {
    ...
    for (std::vector<Expr>::const_iterator next = it + max; it != next;
         ++it, --numChildren) {
...
NodeManager* ExprManager::getNodeManager() const { return d_nodeManager; }
...
Node exportInternal(TNode n, ExprManager* from, ExprManager* to,
...
TypeNode exportTypeInternal(TypeNode n, NodeManager* from, NodeManager* to,
  ...
  if (n.getMetaKind() == kind::metakind::PARAMETERIZED &&
    ...
    throw ExportUnsupportedException(
        "export of PARAMETERIZED-kinded types (other than SORT_KIND) not "
        "supported");
  }
  ...
  if (!to_t.isNull()) {
    Debug("export") << "+ mapped `" << from_t << "' to `" << to_t << "'"
                    << std::endl;
...
Type ExprManager::exportType(const Type& t, ExprManager* em,
  ...
  return Type(em->d_nodeManager,
              new TypeNode(expr::exportTypeInternal(

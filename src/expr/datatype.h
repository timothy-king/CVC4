...
inline Type Datatype::getParameter( unsigned int i ) const {
  TmpCheckArgument(isParametric(), this, "cannot get type parameter of a non-parametric datatype");
  TmpCheckArgument(i < d_params.size(), i, "type parameter index out of range for datatype");
  return d_params[i];
}

inline std::vector<Type> Datatype::getParameters() const {
  TmpCheckArgument(isParametric(), this, "cannot get type parameters of a non-parametric datatype");
  return d_params;
}

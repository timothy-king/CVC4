...
std::string Expr::getString() const {
  CVC4::TmpCheckArgument(getKind() == CVC4::kind::CONST_STRING, *this, "CVC3::Expr::getString(): not a string Expr: `%s'", toString().c_str());
  return getConst<CVC4::String>().toString();
}

std::vector<Expr> Expr::getVars() const {
  CVC4::TmpCheckArgument(isClosure(), *this, "CVC3::Expr::getVars(): not a closure Expr: `%s'", toString().c_str());
...
Expr Expr::getBody() const {
  CVC4::TmpCheckArgument(isClosure(), *this, "CVC3::Expr::getBody(): not a closure Expr: `%s'", toString().c_str());
  return (*this)[1];
}
...
const Rational& Expr::getRational() const {
  CVC4::TmpCheckArgument(isRational(), *this, "CVC3::Expr::getRational(): not a rational Expr: `%s'", toString().c_str());
...
std::vector< std::vector<Expr> > Expr::getTriggers() const {
  CVC4::TmpCheckArgument(isClosure(), *this, "getTriggers() called on non-closure expr");
  if(getNumChildren() < 3) {
...
CLFlag& CLFlag::operator=(bool b) {
  CVC4::TmpCheckArgument(d_tp == CLFLAG_BOOL, this);
  d_data.b = b;
  return *this;
}

CLFlag& CLFlag::operator=(int i) {
  CVC4::TmpCheckArgument(d_tp == CLFLAG_INT, this);
  d_data.i = i;
  return *this;
}

CLFlag& CLFlag::operator=(const std::string& s) {
  CVC4::TmpCheckArgument(d_tp == CLFLAG_STRING, this);
  *d_data.s = s;
  return *this;
}
...
CLFlag& CLFlag::operator=(const std::pair<string, bool>& p) {
  CVC4::TmpCheckArgument(d_tp == CLFLAG_STRVEC, this);
...
const CLFlag& CLFlags::getFlag(const std::string& name) const {
  ...
  CVC4::TmpCheckArgument(i != d_map.end(), name, "No command-line flag by that name, or not supported.");
  return (*i).second;
}
...
void CLFlags::setFlag(const std::string& name, const CLFlag& f) {
  ...
  CVC4::TmpCheckArgument(i != d_map.end(), name, "No command-line flag by that name, or not supported.");
  CVC4::TmpCheckArgument((*i).second.getType() == f.getType(), f,
...
void CLFlags::setFlag(const std::string& name, int i) {
  ...
  CVC4::TmpCheckArgument(it != d_map.end(), name, "No command-line flag by that name, or not supported.");
  (*it).second = i;
}
...
Type ValidityChecker::subrangeType(const Expr& l, const Expr& r) {
  ...
  CVC4::TmpCheckArgument(noLowerBound || (l.getKind() == CVC4::kind::CONST_RATIONAL && l.getConst<Rational>().isIntegral()), l);
  CVC4::TmpCheckArgument(noUpperBound || (r.getKind() == CVC4::kind::CONST_RATIONAL && r.getConst<Rational>().isIntegral()), r);
...
Type ValidityChecker::recordType(const std::vector<std::string>& fields,
  ...
  CVC4::TmpCheckArgument(fields.size() == types.size() && fields.size() > 0,
...
Type ValidityChecker::dataType(const std::string& name,
  ...
  CVC4::TmpCheckArgument(selectors.size() == types.size(), types, "expected selectors and types vectors to be of equal length");
  vector<string> cv;
...
Type ValidityChecker::dataType(const std::string& name,
  ...
  CVC4::TmpCheckArgument(constructors.size() == selectors.size(), selectors, "expected constructors and selectors vectors to be of equal length");
  CVC4::TmpCheckArgument(constructors.size() == types.size(), types, "expected constructors and types vectors to be of equal length");
  vector<string> nv;
...
void ValidityChecker::dataType(const std::vector<std::string>& names,
  ...
  CVC4::TmpCheckArgument(names.size() == constructors.size(), constructors, "expected names and constructors vectors to be of equal length");
  CVC4::TmpCheckArgument(names.size() == selectors.size(), selectors, "expected names and selectors vectors to be of equal length");
  CVC4::TmpCheckArgument(names.size() == types.size(), types, "expected names and types vectors to be of equal length");
  vector<CVC4::Datatype> dv;

  // Set up the datatype specifications.
  for(unsigned i = 0; i < names.size(); ++i) {
    CVC4::Datatype dt(names[i], false);
    CVC4::TmpCheckArgument(constructors[i].size() == selectors[i].size(), "expected sub-vectors in constructors and selectors vectors to match in size");
    CVC4::TmpCheckArgument(constructors[i].size() == types[i].size(), "expected sub-vectors in constructors and types vectors to match in size");
    for(unsigned j = 0; j < constructors[i].size(); ++j) {
      CVC4::DatatypeConstructor ctor(constructors[i][j]);
      CVC4::TmpCheckArgument(selectors[i][j].size() == types[i][j].size(), types, "expected sub-vectors in selectors and types vectors to match in size");
  ...
  for(vector<CVC4::DatatypeType>::iterator i = dtts.begin(); i != dtts.end(); ++i) {
    ...
    for(CVC4::Datatype::const_iterator j = dt.begin(); j != dt.end(); ++j) {
      ...
      CVC4::TmpCheckArgument(d_constructors.find((*j).getName()) == d_constructors.end(), constructors, "cannot have two constructors with the same name in a ValidityChecker");
      d_constructors[(*j).getName()] = &dt;
      for(CVC4::DatatypeConstructor::const_iterator k = (*j).begin(); k != (*j).end(); ++k) {
        CVC4::TmpCheckArgument(d_selectors.find((*k).getName()) == d_selectors.end(), selectors, "cannot have two selectors with the same name in a ValidityChecker");
...
Type ValidityChecker::bitvecType(int n) {
  CVC4::TmpCheckArgument(n >= 0, n, "cannot construct a bitvector type of negative size");
...
Expr ValidityChecker::varExpr(const std::string& name, const Type& type,
                              const Expr& def) {
  CVC4::TmpCheckArgument(def.getType() == type, def, "expected types to match");
...
Expr ValidityChecker::andExpr(const std::vector<Expr>& children) {
  ...
  CVC4::TmpCheckArgument(children.size() > 0, children);
...
Expr ValidityChecker::distinctExpr(const std::vector<Expr>& children) {
  CVC4::TmpCheckArgument(children.size() > 1, children, "it makes no sense to create a `distinct' expression with only one child");
...
Op ValidityChecker::createOp(const std::string& name, const Type& type,
                             const Expr& def) {
  CVC4::TmpCheckArgument(def.getType() == type, type,
...
Expr ValidityChecker::ratExpr(const std::string& n, int base) {
  if(n.find(".") == string::npos) {
    ...
    CVC4::TmpCheckArgument(base == 10, base, "unsupported base for decimal parsing");
...
Expr ValidityChecker::newBVConstExpr(const Rational& r, int len) {
  ...
  CVC4::TmpCheckArgument(r.getDenominator() == 1, r, "ValidityChecker::newBVConstExpr: "
                "not an integer: `%s'", r.toString().c_str());
  CVC4::TmpCheckArgument(len > 0, len, "ValidityChecker::newBVConstExpr: "
                "len = %d", len);
...
Expr ValidityChecker::newConcatExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only concat a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only concat a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVExtractExpr(const Expr& e, int hi, int low) {
  CVC4::TmpCheckArgument(e.getType().isBitVector(), e, "can only bvextract from a bitvector, not a `%s'", e.getType().toString().c_str());
  CVC4::TmpCheckArgument(hi >= low, hi, "extraction [%d:%d] is bad; possibly inverted?", hi, low);
  CVC4::TmpCheckArgument(low >= 0, low, "extraction [%d:%d] is bad (negative)", hi, low);
  CVC4::TmpCheckArgument(CVC4::BitVectorType(e.getType()).getSize() > unsigned(hi), hi, "bitvector is of size %u, extraction [%d:%d] is off-the-end", CVC4::BitVectorType(e.getType()).getSize(), hi, low);
...
Expr ValidityChecker::newBVNegExpr(const Expr& t1) {
  ...
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvneg a bitvector, not a `%s'", t1.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_NOT, t1);
}

Expr ValidityChecker::newBVAndExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvand a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvand a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVAndExpr(const std::vector<Expr>& kids) {
  ...
  CVC4::TmpCheckArgument(kids.size() > 1, kids, "BITVECTOR_AND must have at least 2 children");
...
Expr ValidityChecker::newBVOrExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvor a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvor a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVOrExpr(const std::vector<Expr>& kids) {
  ...
  CVC4::TmpCheckArgument(kids.size() > 1, kids, "BITVECTOR_OR must have at least 2 children");
  std::vector<Expr>::const_reverse_iterator i = kids.rbegin();
  Expr e = *i++;
  while(i != kids.rend()) {
...
Expr ValidityChecker::newBVXorExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvxor a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvxor a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVXorExpr(const std::vector<Expr>& kids) {
  ...
  CVC4::TmpCheckArgument(kids.size() > 1, kids, "BITVECTOR_XOR must have at least 2 children");
  std::vector<Expr>::const_reverse_iterator i = kids.rbegin();
  Expr e = *i++;
  while(i != kids.rend()) {
...
Expr ValidityChecker::newBVXnorExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvxnor a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvxnor a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVXnorExpr(const std::vector<Expr>& kids) {
  ...
  CVC4::TmpCheckArgument(kids.size() > 1, kids, "BITVECTOR_XNOR must have at least 2 children");
  std::vector<Expr>::const_reverse_iterator i = kids.rbegin();
  Expr e = *i++;
  while(i != kids.rend()) {
...
Expr ValidityChecker::newBVNandExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvnand a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvnand a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_NAND, t1, t2);
}

Expr ValidityChecker::newBVNorExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvnor a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvnor a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_NOR, t1, t2);
}

Expr ValidityChecker::newBVCompExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvcomp a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvcomp a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_COMP, t1, t2);
}

Expr ValidityChecker::newBVLTExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvlt a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvlt a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_ULT, t1, t2);
}

Expr ValidityChecker::newBVLEExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvle a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvle a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_ULE, t1, t2);
}

Expr ValidityChecker::newBVSLTExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvslt a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvslt a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_SLT, t1, t2);
}

Expr ValidityChecker::newBVSLEExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvsle a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvsle a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_SLE, t1, t2);
}

Expr ValidityChecker::newSXExpr(const Expr& t1, int len) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only sx a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(len >= 0, len, "must sx by a positive integer");
  CVC4::TmpCheckArgument(unsigned(len) >= CVC4::BitVectorType(t1.getType()).getSize(), len, "cannot sx by something smaller than the bitvector (%d < %u)", len, CVC4::BitVectorType(t1.getType()).getSize());
...
Expr ValidityChecker::newBVUminusExpr(const Expr& t1) {
  ...
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvuminus a bitvector, not a `%s'", t1.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_NEG, t1);
}

Expr ValidityChecker::newBVSubExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvsub a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvsub by a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::bvpad(int len, const Expr& e) {
  CVC4::TmpCheckArgument(len >= 0, len,
                "padding length must be a non-negative integer, not %d", len);
  CVC4::TmpCheckArgument(e.getType().isBitVector(), e,
...
Expr ValidityChecker::newBVPlusExpr(int numbits, const std::vector<Expr>& kids) {
  ...
  CVC4::TmpCheckArgument(kids.size() > 1, kids, "BITVECTOR_PLUS must have at least 2 children");
  std::vector<Expr>::const_reverse_iterator i = kids.rbegin();
  Expr e = *i++;
  while(i != kids.rend()) {
  ...
  unsigned size = CVC4::BitVectorType(e.getType()).getSize();
  CVC4::TmpCheckArgument(unsigned(numbits) == size, numbits,
...
Expr ValidityChecker::newBVPlusExpr(int numbits, const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvplus a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvplus a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVMultExpr(int numbits, const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvmult a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvmult by a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::newBVUDivExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvudiv a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvudiv by a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_UDIV, t1, t2);
}

Expr ValidityChecker::newBVURemExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvurem a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvurem by a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_UREM, t1, t2);
}

Expr ValidityChecker::newBVSDivExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvsdiv a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvsdiv by a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_SDIV, t1, t2);
}

Expr ValidityChecker::newBVSRemExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvsrem a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvsrem by a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_SREM, t1, t2);
}

Expr ValidityChecker::newBVSModExpr(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only bvsmod a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only bvsmod by a bitvector, not a `%s'", t2.getType().toString().c_str());
  return d_em->mkExpr(CVC4::kind::BITVECTOR_SMOD, t1, t2);
}

Expr ValidityChecker::newFixedLeftShiftExpr(const Expr& t1, int r) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only left-shift a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(r >= 0, r, "left shift amount must be >= 0 (you passed %d)", r);
  // Defined in:
...
Expr ValidityChecker::newFixedConstWidthLeftShiftExpr(const Expr& t1, int r) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only right-shift a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(r >= 0, r, "const-width left shift amount must be >= 0 (you passed %d)", r);
...
Expr ValidityChecker::newFixedRightShiftExpr(const Expr& t1, int r) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only right-shift a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(r >= 0, r, "right shift amount must be >= 0 (you passed %d)", r);
  // Defined in:
  // http://www.cs.nyu.edu/acsys/cvc3/doc/user_doc.html#user_doc_pres_lang_expr_bit
...
Expr ValidityChecker::newBVSHL(const Expr& t1, const Expr& t2) {
  CVC4::TmpCheckArgument(t1.getType().isBitVector(), t1, "can only right-shift a bitvector, not a `%s'", t1.getType().toString().c_str());
  CVC4::TmpCheckArgument(t2.getType().isBitVector(), t2, "can only right-shift by a bitvector, not a `%s'", t2.getType().toString().c_str());
...
Expr ValidityChecker::tupleSelectExpr(const Expr& tuple, int index) {
  CVC4::TmpCheckArgument(index >= 0 && index < tuple.getNumChildren(),
...
Expr ValidityChecker::datatypeConsExpr(const std::string& constructor, const std::vector<Expr>& args) {
  ...
  CVC4::TmpCheckArgument(i != d_constructors.end(), constructor, "no such constructor");
  const CVC4::Datatype& dt = *(*i).second;
  const CVC4::DatatypeConstructor& ctor = dt[constructor];
  CVC4::TmpCheckArgument(ctor.getNumArgs() == args.size(), args, "arity mismatch in constructor application");
...
Expr ValidityChecker::datatypeSelExpr(const std::string& selector, const Expr& arg) {
  ...
  CVC4::TmpCheckArgument(i != d_selectors.end(), selector, "no such selector");
...
void ValidityChecker::getUserAssumptions(std::vector<Expr>& assumptions) {
  CVC4::TmpCheckArgument(assumptions.empty(), assumptions, "assumptions arg must be empty");
...
FormulaValue ValidityChecker::value(const Expr& e) {
  CVC4::TmpCheckArgument(e.getType() == d_em->booleanType(), e, "argument must be a formula");
  try {
...
bool ValidityChecker::inconsistent(std::vector<Expr>& assumptions) {
  CVC4::TmpCheckArgument(assumptions.empty(), assumptions, "assumptions vector should be empty on entry");
...
void ValidityChecker::popto(int stackLevel) {
  CVC4::TmpCheckArgument(stackLevel >= 0, stackLevel,
                      "Cannot pop to a negative stack level %d", stackLevel);
  CVC4::TmpCheckArgument(unsigned(stackLevel) <= d_stackLevel, stackLevel,
...
void ValidityChecker::poptoScope(int scopeLevel) {
  CVC4::TmpCheckArgument(scopeLevel >= 0, scopeLevel,
                      "Cannot pop to a negative scope level %d", scopeLevel);
  CVC4::TmpCheckArgument(unsigned(scopeLevel) <= d_parserContext->scopeLevel(),
                      scopeLevel,

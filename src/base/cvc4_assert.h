...
class AssertArgumentException : public AssertionException {
 ...
 public:
  AssertArgumentException(const char* argDesc, const char* function,
    ...
    va_list args;
    va_start(args, fmt);
    const std::string description = MakeArgumentDescription(argDesc);
    construct("Illegal argument detected", description.c_str(), function, file,
              line, fmt, args);
    va_end(args);
  }
  ...
  AssertArgumentException(const char* condStr, const char* argDesc,
    ...
    va_list args;
    va_start(args, fmt);
    const std::string description = MakeArgumentDescription(argDesc, condStr);
    construct("Illegal argument detected", description.c_str(), function, file,
              line, fmt, args);
    va_end(args);
  }
 ...
 private:
  static std::string MakeArgumentDescription(const char* argDesc,
                                             const char* condStr = NULL) {
    std::stringstream description_stream("`");
    description_stream << argDesc << "' is a bad argument";
    if (condStr != NULL) {
      description_stream << "; expected " << condStr << " to hold";
    }
...
#define Unreachable()                                                   \
  throw ::CVC4::UnreachableCodeException(__PRETTY_FUNCTION__, __FILE__, \
                                         __LINE__)
#define UnreachableWithMessage(msg...)                                  \
  throw ::CVC4::UnreachableCodeException(__PRETTY_FUNCTION__, __FILE__, \
                                         __LINE__, msg)
